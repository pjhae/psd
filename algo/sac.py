...
class SAC(object):
    ...
    def update_parameters_metra(self, memory, batch_size, updates, phi):
    ...
    def update_parameters(self, memory, psi, updates, args):
        ...
        # 'ij,ij->i' means take the dot product along the second dimension for each i in the first dimension
        pseudo_reward_batch = -torch.norm(radius_batch*torch.sin(torch.tensor(np.pi/(2*radius_batch)).clone().detach()) - torch.norm(psi_diff, p=2, dim=-1), p=2, dim=-1).unsqueeze(1)
